...
static x264_t* init_encoder(int width, int height, int fps, x264_param_t* param) {
    ...
    // Çok küçük slice boyutu bazı cihazlarda overhead yaratabilir; yine de senin tercihin.
    param->i_slice_max_size = 1000;

    // ABR + VBV
    param->rc.i_rc_method = X264_RC_ABR;
    param->rc.i_bitrate = 1800;          // başlangıç
    param->rc.i_vbv_max_bitrate = 2200;  // tavan
    param->rc.i_vbv_buffer_size = 1200;  // buffer
...
// 🔥 YENİ FONKSİYON: CANLI BITRATE GÜNCELLEME
...
// SetBitrate: Canlı yayında kaliteyi değiştirir (thread-safe).
func (e *Encoder) SetBitrate(kbps int) {
	// Güvenli clamp (senin engine tarafındaki seviyelere uyumlu)
	if kbps < 300 {
		kbps = 300
	}
	if kbps > 8000 {
	...
	// Çok sık reconfig encoderı yorabilir → min aralık
	if time.Since(e.lastReconf) < 750*time.Millisecond {
		return
	}

	// Aynı bitrate’i tekrar tekrar basma
...
func (e *Encoder) Encode(img *image.RGBA) []byte {
	...
	// ✅ KRİTİK DÜZELTME:
	// NAL'lar contiguous garanti değildir. Bu yüzden her NAL payload'ı ayrı ayrı kopyalanır.
	nalSlice := unsafe.Slice(nals, int(iNals))

	// toplam boyutu güvenli hesapla
	total := 0
	for i := 0; i < int(iNals); i++ {
		if nalSlice[i].i_payload > 0 && nalSlice[i].p_payload != nil {
